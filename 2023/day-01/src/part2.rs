...
fn process_line(line: &str) -> u32 {
    ...
    let nums: Vec<u32> = (0..line_len) //iterate for the length of `line`
        .filter_map(|index| {
            let ch = line.as_bytes()[index]; //the character at the index `index` of this line.
            if ch.is_ascii_digit() {
                return Some(
                    (ch as u32) - 48, // -48 for the ascii offset
                );
            }
            ...
            NUMBERWORDS.iter().enumerate().find_map(|(i, numberword)| {
                ...
                if (line_len - index) >= 3 && line[index..].starts_with(numberword) {
                    Some(
                        (i + 1) as u32, // +1 as NUMBERWORDS starts with index 0, but the first term is "one"
                    )
                } else {
